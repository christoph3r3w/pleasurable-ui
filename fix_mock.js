...
app.get("/detail/:id", (req, res) => {
    ...
    console.log(\??? GET /detail/\ (Rendering detail page)\);
    Promise.all([
        fetchJson(\\/posts/\\),
        fetchJson(categoriesUrl),
        fetchJson(\\?filter[id][_eq]=\\)
...
app.post("/detail/:id/likes", (req, res) => {
    fetchJson(\\/?filter[id][_eq]=\\).then(({data}) => {
...
app.listen(PORT, () => {
    ...
    console.log(\   GET http://localhost:\/wp-json/wp/v2/...\n\);
    console.log('?? Frontend Routes:');
    console.log(\   - Home:         http://localhost:\/\);
    console.log(\   - Detail:       http://localhost:\/detail/:id\);
    console.log(\   - Authors:      http://localhost:\/authors\);
    console.log(\   - Categories:   http://localhost:\/category/:name\);
});
